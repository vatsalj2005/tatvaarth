...
const ShastraLanding = () => {
  ...
  // Category Shastras for page grid
  ...
  const searchScope = useMemo(() => ({
    categorySlug,
    pathPrefix: categorySlug ? `/shastra/${categorySlug}` : '/shastra',
    label: currentCategory
      ? (language === 'hi' ? currentCategory.nameHi : currentCategory.nameEn)
      : (language === 'hi' ? 'शास्त्र संग्रह' : 'Scriptures')
  }), [categorySlug, currentCategory, language]);

  const searchPlaceholder = language === 'hi' ? 'ग्रन्थ या लेखक खोजें...' : 'Search scriptures or authors...';

  // Render the list of all categories if categorySlug is undefined
  if (!categorySlug) {
    return (
  ...
  // Render Shastras in active category
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <Header />
      <div className="pt-28 pb-16 px-4 flex-1">
